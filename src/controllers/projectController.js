...
const getAllprojects = async (req, res) => {
  try {
    ...
    if (response) {
      return controller.response(res, "Lấy dữ liệu thành công", 200, response, true);
    } else {
      return controller.response(res, "Lấy dữ liệu thất bại", 200, response, false);
    }
  } catch (error) {
    return controller.response(res, error.message, error.status, response, false);
  }
...
module.exports = { handleCreateProject, getAllprojects };
